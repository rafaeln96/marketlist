...
let editIndex = null; // Índice do item em edição
...
// Carrega dados de um item no formulário para edição
function editQuickItem(index) {
    const item = quickItems[index];
    editIndex = index;

    // Alterna para o modo correto do item
    if (item.type === 'weight') {
        setQuickMode('weight');
        document.getElementById('quick-item-name-weight').value = item.name;
        // Extrai o valor numérico do peso (ex: "1,250kg" -> "1,250")
        const weightStr = item.displayQty.replace('kg', '');
        document.getElementById('quick-item-weight').value = weightStr;
    } else {
        setQuickMode('qty');
        document.getElementById('quick-item-name').value = item.name;
        // Extrai o valor numérico da quantidade (ex: "3x" -> 3)
        const qty = parseInt(item.displayQty.replace('x', '')) || 1;
        document.getElementById('quick-item-qty').value = qty;
    }

    // Mostra botões de atualizar/cancelar, esconde adicionar
    document.getElementById('add-quick-btn').style.display = 'none';
    document.getElementById('update-quick-btn').style.display = 'flex';
    document.getElementById('cancel-quick-btn').style.display = 'flex';

    // Scroll para o formulário
    document.getElementById('quick-list-form').scrollIntoView({ behavior: 'smooth', block: 'center' });
}

// Atualiza o item em edição
function updateQuickItem() {
    if (editIndex === null) return;

    let name, displayQty, type;
    const item = quickItems[editIndex];
    ...
    if (currentMode === 'qty') {
        ...
        // Verifica duplicatas (exceto o próprio item)
        if (quickItems.some((it, i) => i !== editIndex && it.name.toLowerCase() === name.toLowerCase() && !it.checked)) {
            showNotification(`"${name}" já está na lista!`, 'error');
            return;
        }
    } else {
    ...
    // Atualiza o item (mantém o estado checked)
    quickItems[editIndex] = {
        name: name,
        displayQty: displayQty,
        type: type,
...
// Cancela modo de edição e restaura formulário
function cancelQuickEdit() {
    ...
    document.getElementById('quick-item-name-weight').value = '';
    document.getElementById('quick-item-weight').value = '';

    // Restaura botões
    document.getElementById('add-quick-btn').style.display = 'flex';
    document.getElementById('update-quick-btn').style.display = 'none';
    document.getElementById('cancel-quick-btn').style.display = 'none';
}
    ...
    sortedItems.forEach((item) => {
        ...
        itemDiv.innerHTML = `
            ...
            <div class="quick-item-actions">
                <button class="quick-action-btn quick-edit-btn" onclick="editQuickItem(${originalIndex})" title="Editar">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('quick-item-weight').addEventListener('keypress', function (e) {
        ...
        // Bloquear letras no campo de peso (apenas dígitos e vírgula permitidos)
        if (!/[0-9,]/.test(e.key)) {
            e.preventDefault();
        }
    });
